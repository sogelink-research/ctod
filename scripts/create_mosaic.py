...
def calculate_extent(tiff_file, ignore_nodata):
    ...
    if not ignore_nodata:
        ...
        print(
            f"Calculating extent excluding nodata, this can take some time: {tiff_file}"
        )
...
def create_json(input_folder, output_json, ignore_nodata, overwrite_path):
    ...
    for filename in os.listdir(input_folder):
        if filename.lower().endswith((".tif", ".tiff")):
            ...
            datasets.append({"path": ds_path, "extent": [left, bottom, right, top]})
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate JSON file from Cloud Optimized GeoTIFFs."
    )
    parser.add_argument(
        "-i",
        "--input",
    ...
    parser.add_argument(
        ...
        "--output",
        metavar="output_file",
        default="dataset.ctod",
        help="Specify the output JSON file name.",
    )
    parser.add_argument(
        "-op",
        "--overwrite-path",
        metavar="overwrite_path",
        default=None,
