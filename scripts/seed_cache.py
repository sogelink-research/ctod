...
def seed_cache(tms: TileMatrixSet, input_filepath: str, output_filepath: str, meshing_method: str, params: str, zoom_levels: list, overwrite: bool, done_future: Future):
    create_cache_folder(output_filepath)
    layer_json = get_layer_json(tms, input_filepath)

    for zoom in zoom_levels:        
        generate_level(tms, input_filepath, output_filepath, layer_json, zoom, meshing_method, params, overwrite)
    
    logging.info(f"Finished seeding cache, stopping...")
    done_future.set_result(None)
    
def generate_level(tms: TileMatrixSet, input_filepath: str, output_filepath: str, layer_json: dict, zoom: int, meshing_method: str, params: str, overwrite: bool):
    ...
    logging.info(f"Generating cache for zoom level {zoom} with {len(x_range) * len(y_range)} tile(s)")
    
    for x in x_range:
        for y in y_range:
            generate_tile(input_filepath, output_filepath, tms_y_max, x, y, zoom, meshing_method, params, overwrite)
    
def generate_tile(input_filepath: str, output_filepath: str, tms_y_max: int, x: int, y: int, z: int, meshing_method: str, params: str, overwrite: bool):
    # If overwrite is false, skip generating and caching the tile
    if not overwrite:
        cached_tile = get_tile_from_disk(output_filepath, input_filepath, meshing_method, params, z, x, y)
    ...
    if response.status_code == 200:
        ...
        y = tms_y_max - y
        save_tile_to_disk(output_filepath, input_filepath, meshing_method, z, x, y, tile_data)
    else:
        logging.error(f"Failed to generate tile {x} {y} {z}. Status code: {response.status_code}")
    
if __name__ == "__main__":
    setup_logging()
    parser = argparse.ArgumentParser(description="Seed the cache")
    parser.add_argument("-i", "--input", metavar="input_dataset", required=True,
    ...
    parser.add_argument("-o", "--output", metavar="output_folder", default="./cache",
        help="Specify the output folder for the cache.")
    parser.add_argument("-m", "--meshing-method", metavar="meshing_method", default="grid",
        help="The meshing method to use: grid, delatin, martini. Defaults to grid.")
    parser.add_argument("-z", "--zoom-levels", metavar="zoom_levels", required=True,
        help="The zoom levels to create a cache for. Separate multiple levels with '-'.")
    parser.add_argument("-p", "--params", metavar="request_parameters", required=False, default=None,
    ...
    done_future = Future()
    threading.Thread(target=seed_cache, args=(tms, args.input, args.output, args.meshing_method, args.params, zoom_levels, args.overwrite, done_future)).start()
